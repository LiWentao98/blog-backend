...
import com.mtli.schedule.BlogTask;
import com.mtli.utils.*;
...
//    private FileUtil fileUtil;
    ...
    public void saveBlog(String blogTitle, String blogBody, Integer[] tagIds) throws JsonProcessingException {
        ...
        // 留出两个位置，避免并发量稍高一点，造成超出限制
        if(redisTemplate.opsForList().size(RedisConfig.REDIS_NEW_BLOG) >= RedisConfig.REDIS_NEW_BLOG_COUNT-2){
            // 避免超出，达到这个数量，就rightpop一个
    ...
    public List<Blog> findHomeBlog(Integer page, Integer showCount) throws IOException {
        ...
        if (!redisTemplate.hasKey(RedisConfig.REDIS_NEW_BLOG)) {
            ...
            // 0-10
            List<Blog> blogFromMysql = blogDao.findHomeBlog(0, RedisConfig.REDIS_NEW_BLOG_COUNT);
            for (Blog blogs : blogFromMysql) {
                String blogId = Integer.toString(blogs.getId());
                blogs.setTags(tagDao.findTagByBlogId(blogs.getId()));


                redisTemplate.opsForList().rightPush(RedisConfig.REDIS_NEW_BLOG, blogId);
                redisTemplate.opsForValue().set(RedisConfig.REDIS_BLOG_PREFIX + blogId, objectMapper.writeValueAsString(blogs));
            }
        }
